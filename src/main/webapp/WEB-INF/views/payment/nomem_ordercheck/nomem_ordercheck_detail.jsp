...
${order_detail_list[0].send_name }님께서 ${fn:substring(order_detail_list[0].rdate,0,4) }년 ${fn:substring(order_detail_list[0].rdate,5,7) }월 ${fn:substring(order_detail_list[0].rdate,8,10) }일에 주문하신 내역입니다.
</div>
...
<tr>
	<td>
	<c:if test="${order_detail_list[0].payment_method == 'vbank' }">무통장입금</c:if>
	...
	<td>
	<c:if test="${order_detail_list[0].payment_status =='ready' }">미결제</c:if>
	...
	<td>
	<input type="button" id="ordercheck_delivery_btn" onclick="ordercheck_delivery(${order_detail_list[0].payment_code })" value="배송조회">
	</td> --%>
</tr>
</table>
</div>
